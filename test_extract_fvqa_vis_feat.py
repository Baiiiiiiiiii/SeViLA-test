...
split='test'
max_feats = 10 # frame per video
output_path = f"/home/eric/temp/SeViLA-test/FVQA_{split}_ViT14_feats.pt"



data = json.load(open(f'/home/eric/temp/SeViLA-test/dataset/STAR_{split}.json', 'r'))
features = torch.load(f'/home/eric/temp/SeViLA-test/clipvitl14.pth')
features_dim = 768


def get_video(video_id, start, end):
    if video_id not in features:
        print(video_id)
        video = torch.zeros(1, features_dim)
    else:
        video = features[video_id][start: end +1, :].float() # ts
    if len(video) > max_feats:
        ...
        video_len = len(video)
        video = torch.cat([video, torch.zeros(max_feats - video_len, features_dim)], 0)
    else:
        video_len = max_feats
...
# def get_vis_feat(self, idx):
dict={}
for idx in range(len(data)):
    vid = data[idx]['video_id']
    qid = data[idx]['question_id']
    start, end = round(data[idx]['start']), round(data[idx]['end'])
    video, video_len = get_video(f'{vid}', start, end)
    dict[qid] = video


# print(dict["Feasibility_T6_1468"])
# print(dict["Feasibility_T6_1468"].size())
