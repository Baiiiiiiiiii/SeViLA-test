...
class SeViLA(Blip2Base):
    ...
    def __init__( self, img_size=224, drop_path_rate=0,
        ...
        self.addfeat_proj = nn.Linear(self.addfeat_hidden_size ,self.Qformer.config.hidden_size)
        ...
        self.vid_prefix = ['Frame {}: '.format(str(i+1)) for i in range(frame_num+8)]
    ...
    def forward(self, samples,
        ...
        # Finetune answerer with localizer
            ...
            sf = select_frames.reshape(b, -1, select_frames.shape[-2], select_frames.shape[-1]) ###
            # print(sf.size())
            # print(addfeat.size())
            addfeat = addfeat.reshape(b, -1, select_frames.shape[-2], addfeat.shape[-1])# b*8, ###
            addfeat_tf = self.addfeat_proj(addfeat) ###
            qt_sf_addf = torch.cat([sf, addfeat_tf], dim=1) ###
            qt_sf_addf = qt_sf_addf.reshape(-1, select_frames.shape[-2], select_frames.shape[-1]) ###
            ...
            query_tokens_qa = self.query_tokens.expand(qt_sf_addf[0], -1, -1) #select_frames.shape[0], -1, -1 ###
            atts_sf_addf = torch.ones(query_tokens_qa.size()[:-1], dtype=torch.long).to(image.device)#torch.cat([image_atts, addf_tf_atts], dim=0) ###
            query_output_qa = self.Qformer.bert(
                query_embeds=query_tokens_qa, encoder_hidden_states=select_frames,
                encoder_attention_mask=atts_sf_addf, return_dict=True)
            inputs_t5_qa = self.t5_proj(query_output_qa.last_hidden_state)
            ...
            with torch.cuda.amp.autocast(dtype=torch.bfloat16):        
                ...
                inputs_embeds_qa = torch.cat([inputs_t5_qa, inputs_embeds_qa], dim=1) #cat([inputs_t5_qa, ego4d_feat, inputs_embeds_qa], dim=1)
                encoder_atts_qa = torch.cat([atts_t5_qa, input_tokens_qa.attention_mask], dim=1) #cat([atts_t5_qa, atts_ego4d, input_tokens_qa.attention_mask], dim=1)
